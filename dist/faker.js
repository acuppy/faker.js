...
Faker.Util = {
  ...
  interpret: function(raw) {
    var alphaTemplates, blocksToInterpret,
      _this = this;
    alphaTemplates = raw.match(/(?:(#{[a-zA-Z\.\-\_]+})|(#(?!{)))/gi) || [];
    blocksToInterpret = alphaTemplates;
    $.each(blocksToInterpret, function(index, elm) {
      ...
      return raw = _this.mapConcat(raw, function(char) {
        return char.replace(/#(?!{)/gi, Faker.Util.Numbers.sample(0, 9));
      });
    });
    return raw;
  },
  fixUmlauts: function(string) {
    return this.arrayReplace(string, ["ä", 'ae'], ["ö", 'oe'], ["ü", 'ue'], ["ß", 'ss']);
  },
  fixNonWordChars: function(string) {
    return string.replace(/\W/gi, '');
  },
  mapConcat: function() {
    var args, char, chars, i, rtn, separator, str, _i, _len;
...
Faker.Util.Numbers = {
  ...
  sample: function(start, end) {
    return Faker.Util.Random.sample(this.range.apply(this, arguments));
  }
};

Faker.Util.Alpha = {
  range: function(start, end) {
    var chars, index, range, _i;
...
Faker.Locale = {
  ...
  collection: function(key) {
    var arrPath, collection, i, _i, _ref;
    collection = Faker.Locales[Faker.config.locale];
    arrPath = key.split(".");
    for (i = _i = 0, _ref = arrPath.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
      collection = collection[arrPath[i]];
    }
    if (Faker.Util.isBlank(collection)) {
      $.error("Unknown key: \"" + (key.toString()) + "\" -- you may need to scope it to the proper locale library (e.g. \"name.first_name\")");
    }
    return collection;
  },
  ...
  extend: function() {
    var _base;
    if ((_base = Faker.Locales)[id] == null) {
      _base[id] = {};
    }
...
Faker.Company = Faker.extend({
  name: function() {
    return Faker.Util.interpret(this.sample("company.name"));
  },
  suffix: function() {
    return this.sample("company.suffix");
  },
  catchPhrase: function() {
    return $.map(this.collection("company.buzzwords"), function(elm, i) {
      return Faker.Util.Random.sample(elm);
    }).join(' ');
  },
  bs: function() {
    return $.map(this.collection("company.bs"), function(elm, i) {
      return Faker.Util.Random.sample(elm);
    }).join(' ');
  }
});
...
Faker.Internet = Faker.extend({
  email: function() {
    return [this.userName(), this.domainName()].join('@');
  },
  freeEmail: function() {
    return [this.userName(), this.sample('internet.freeEmail')].join('@');
  },
  safeEmail: function() {
    return [this.userName(), "example." + (Faker.Util.Random.sample(['com', 'net', 'org']))].join('@');
  },
  userName: function() {
    var samples;
    samples = [
      (function() {
        return Faker.Util.fixNonWordChars(Faker.Name.firstName());
      }), (function() {
        return $.map([Faker.Name.firstName(), Faker.Name.lastName()], function(elm, index) {
          return Faker.Util.fixNonWordChars(elm);
        }).join('_').toLowerCase();
      })
    ];
    return Faker.Util.fixUmlauts(Faker.Util.Random.sample(samples).call());
  },
  domainName: function() {
    return [Faker.Util.fixUmlauts(this.domainWord()), this.domainSuffix()].join('.');
  },
  domainWord: function() {
    return Faker.Util.fixNonWordChars(Faker.Company.name().split(' ')[0]).toLowerCase();
  },
  domainSuffix: function() {
    return this.sample('internet.domainSuffix');
  },
  ipV4Address: function() {
    var address, i, range, _i;
...
Faker.Lorem = Faker.extend({
  word: function() {
    return this.sample("lorem.words");
  },
  words: function(qty) {
    if (!$.isNumeric(qty)) {
      $.error("Quantity must be a valid integer");
    }
    return this.sample("lorem.words", qty);
  },
  characters: function(qty, asArray) {
    var arr;
    if (!$.isNumeric(qty)) {
      $.error("Quantity must be a valid integer");
    }
    arr = this.sample("lorem.characters", qty);
    if (asArray === true) {
      return arr;
    } else {
      return arr.join('');
  ...
  sentence: function(options) {
    ...
    sentence = arr.join(' ');
    if (opt.endWithPeriod === true) {
      sentence += '.';
    }
    return sentence;
  },
  sentences: function(qty) {
    var args, asArray, i, options, sentences, _i;
    if (!$.isNumeric(qty)) {
      $.error("Quantity must be a valid integer");
    }
  ...
  paragraphs: function(qty) {
    var args, asArray, i, options, paragraphs, _i;
    if (!$.isNumeric(qty)) {
      $.error("Quantity must be a valid integer");
    }
    args = Array.prototype.slice.call(arguments, 1);
...
Faker.Name = Faker.extend({
  ...
  prefix: function() {
    return this.sample('name.prefix');
  },
  firstName: function() {
    return this.sample('name.firstName');
  },
  lastName: function() {
    return this.sample('name.lastName');
  },
  suffix: function() {
    return this.sample('name.suffix');
  },
  title: function() {
    var title;
    title = new Array();
    title.push(this.sample('name.title.descriptor'));
    title.push(this.sample('name.title.level'));
    title.push(this.sample('name.title.job'));
    return title.join(' ');
  }
});
...
Faker.PhoneNumber = Faker.extend({
  phoneNumber: function() {
    return Faker.Util.interpret(this.sample('phoneNumber.formats'));
  },
  tollFree: function() {}
});

jQuery.fn.faker = function(extension) {
  return this.each(function() {});
};

Faker.Locale.register("en", {
  lorem: {
  ...
  company: {
    ...
    name: ["" + name.last_name + " " + this.suffix, "" + name.last_name + "-" + name.last_name, "" + name.last_name + ", " + name.last_name + " and " + name.last_name]
  },
  internet: {
    freeEmail: ["gmail.com", "yahoo.com", "hotmail.com"],
    domainSuffix: ["com", "biz", "info", "name", "net", "org"]
  },
  address: {},
  phoneNumber: {
    formats: ["###-###-####", "(###)###-####", "1-###-###-####", "###.###.####", "###-###-####", "(###)###-####", "1-###-###-####", "###.###.####"],
    tollFree: ["800", "888", "866", "855", "877"],
    extensions: ["x###", "x####", "x#####", "ext. ###", "ext. ####", "ext. #####"]
  }
});
