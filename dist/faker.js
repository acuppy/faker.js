(function() {
  ...
  Faker.Lorem = Faker.extend({
    ...
    sentences: function(qty) {
      var args, as_array, i, options, sentences, _i;
      if (!$.isNumeric(qty)) {
        $.error("Quantity must be a valid integer");
      }
    ...
    paragraphs: function(qty) {
      var args, as_array, i, options, paragraphs, _i;
      if (!$.isNumeric(qty)) {
        $.error("Quantity must be a valid integer");
      }
      args = Array.prototype.slice.call(arguments, 1);
  ...
  Faker.PhoneNumber = Faker.extend({
    phone_number: function() {
      return Faker.Util.interpret(Faker.Locale.sample('phone_number.formats'));
    },
    toll_free: function() {}
  });
  ...
  Faker.Util = {
    ...
    interpret: function(raw) {
      var alpha_templates, blocksToInterpret;
      alpha_templates = raw.match(/(?:(#{[a-zA-Z\.\-\_]+})|(#(?!{)))/gi) || [];
      blocksToInterpret = alpha_templates;
      $.each(blocksToInterpret, function(index, elm) {
        ...
        return raw = $.mapConcat(raw, function(char) {
    ...
    fix_umlauts: function(string) {
      return $.arrayReplace(string, ["ä", 'ae'], ["ö", 'oe'], ["ü", 'ue'], ["ß", 'ss']);
    },
...
(function() {
  ...
  this.Faker = $.extend(true, {}, this.Faker.Base);
...
(function() {
  ...
  this.Faker.Lorem = this.Faker.extend({
    word: function() {
      return this.Faker.Locale.sample("lorem.words");
    },
    words: function(qty) {
      if (!$.isNumeric(qty)) {
        $.error("Quantity must be a valid integer");
      }
      return this.Faker.Locale.sample("lorem.words", qty);
    },
    characters: function(qty, as_array) {
      var arr;
      if (!$.isNumeric(qty)) {
      ...
      arr = this.Faker.Locale.sample("lorem.characters", qty);
      if (as_array === true) {
        return arr;
      } else {
        return arr.join('');
...
(function() {
  ...
  this.Faker.Name = this.Faker.extend({
    full_name: function() {
      var name;
      name = new Array();
      if (this.Faker.Util.Random.bool()) {
        name.push(this.prefix());
      }
      name.push(this.first_name());
      name.push(this.last_name());
    ...
    prefix: function() {
      return this.Faker.Locale.sample('name.prefix');
    },
    first_name: function() {
      return this.Faker.Locale.sample('name.first_name');
    },
    last_name: function() {
      return this.Faker.Locale.sample('name.last_name');
    },
    suffix: function() {
      return this.Faker.Locale.sample('name.suffix');
    },
    title: function() {
      var title;
      title = new Array();
      title.push(this.Faker.Locale.sample('name.title.descriptor'));
      title.push(this.Faker.Locale.sample('name.title.level'));
      title.push(this.Faker.Locale.sample('name.title.job'));
      return title.join(' ');
    }
  });
...
(function() {
  Faker.Locale.register("en", {
    ...
    company: {
      ...
      name: ["" + name.last_name + " " + company.suffix, "" + name.last_name + "-" + name.last_name, "" + name.last_name + ", " + name.last_name + " and " + name.last_name]
    },
    internet: {
